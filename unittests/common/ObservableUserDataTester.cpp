...
/// Unit tests for server-side observe helpers used by remote_rest
/// (`observe-mode: server-side`) and Fermioniq.
...
TEST(ObservableUserDataTester, serverSideExpectationUsesGlobalRegister) {
  // processResults for observe-mode backends return ExecutionResult(double).
  // async_observe_result::get() must accept has_expectation() on the global
  // register without requiring spin_op->to_string() as the register name.
